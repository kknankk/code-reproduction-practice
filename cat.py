...
class CATBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'before attention x size {x.shape}')
...
class CATLayer1(nn.Module):
    ...
    def __init__(self, dim=818, input_resolution=[14,14], depth=3, num_heads=1, patch_size=14, mlp_ratio=4., qkv_bias=True, 
                 qk_scale=None, drop=0., ipsa_attn_drop=0., cpsa_attn_drop=0., drop_path=0., 
                 norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
        print(f'after cross attention,the size of mul_feature is {x.shape}')
        # x1=x.flatten(2)
        # total_head=nn.Linear(x1.size(1),20)
        # x2=total_head(x1)
        # print(f'after class head,the size of mul_feature is {x2.shape}')  
        # class_head = nn.Linear(self.dim, 20) 
        x1=x.flatten(1)
        # print(x1.shape)
        class_head = nn.Linear(x1.size(1), 20) 
        final_label=class_head(x1)
        print(f'final label size is{final_label.shape}')
      
        return x,final_label
...
#write a new class by myself

class INVARIENT_CAT(nn.Module):
    """ Implementation of "CAT: Cross Attetion in Vision Transformer".
    ...
    def __init__(self, img_size=224, patch_emb_size=16, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=3, num_heads=3,
                 patch_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., ipsa_attn_drop=0., cpsa_attn_drop=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, slice_emb=False,
                 use_checkpoint=False, **kwargs):
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = CATLayer(dim=int(embed_dim),
